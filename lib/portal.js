...
module.exports = function(options) {
  ...
  var sessionOptions = {
    name: '4front-portal',
    proxy: true,
    // Deliberately not setting the path option of the cookie. It seems to be interfering with other session providers.
    cookie: {httpOnly: true, secure: options.https },
  ...
  portal.post('/login', bodyParser.urlencoded({extended: false}), function(req, res, next) {
    ...
    for (var providerName in req.app.settings.identityProviders) {
      if (req.app.settings.identityProviders[providerName].default === true) {
    ...
    req.app.settings.login(req.body.username, req.body.password, identityProvider, function(err, user) {
